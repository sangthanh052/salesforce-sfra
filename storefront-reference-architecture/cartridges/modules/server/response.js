...
/**
 * Get HTTP Headers custom preferences value
 * @returns {Object|null} - HTTP Headers object or null
 */
function getCustomHTTPHeadersConfig() {
    var Site = require('dw/system/Site');
    var sitePrefHTTPHeadersConfigJSON = Site.getCurrent().getCustomPreferenceValue('httpHeadersConfiguration');
    var sitePrefHTTPHeadersConfig;
    // eslint-disable-next-line no-undef
    if (!empty(sitePrefHTTPHeadersConfigJSON)) {
        try {
            sitePrefHTTPHeadersConfig = JSON.parse(sitePrefHTTPHeadersConfigJSON);
        } catch (error) {
            return null;
        }
    }

    return sitePrefHTTPHeadersConfig;
}
...
function Response(response) {
    ...
    var sitePrefHTTPHeadersConfig = getCustomHTTPHeadersConfig();
    // eslint-disable-next-line no-undef
    if (!empty(sitePrefHTTPHeadersConfig) && sitePrefHTTPHeadersConfig.length) {
        sitePrefHTTPHeadersConfig.forEach(function (httpHeader) {
